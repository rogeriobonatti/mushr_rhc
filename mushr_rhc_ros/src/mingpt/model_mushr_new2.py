...
class GPTConfig:
    ...
    def __init__(self, block_size, max_timestep, **kwargs):
...
class GPT(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.embed_timestep_global = nn.Embedding(config.max_timestep*2, config.n_embd) # unique for each token
        self.embed_timestep_local = nn.Embedding(config.max_timestep, config.n_embd) # is the same for S_t and a_t
        ...
        # for the action
        # simple action embedding as single linear layer
        ...
        # add map decoder
        # from almost all the tokens of s_0:t and a_0:t-1, predict the overall map centered in the middle pose of the vehicle
        # a_t is not taken into account because it's still being predicted in deployment and does not affect the map
        encoded_feat_dim = config.n_embd * (config.block_size - 1)
        ...
        # we don't predict the next state directly because it's potentially an image and that would require large capacity
        self.predict_state_token = nn.Sequential(
                              nn.Linear(2*config.n_embd, config.n_embd)
        )

        # from the tokens of s_t, s_t-1 a_t-1, predict delta in pose from t-1 to t
        self.predict_pose_linear = nn.Sequential(
                              nn.Linear(3*config.n_embd, 3)
        )
    ...
    def configure_optimizers_frozen(self, train_config):
        ...
        non_frozen_params_list = list(self.map_decoder.parameters()) + \
                      list(self.predict_action.parameters()) + \
                      list(self.predict_pose_linear.parameters()) + \
                      list(self.predict_pose_deep.parameters()) + \
    ...
    def configure_optimizers(self, train_config):
        ...
        no_decay.add('embed_timestep_global.weight')
        no_decay.add('embed_timestep_local.weight')
    ...
    def forward(self, states, actions, targets=None, gt_map=None, timesteps=None, poses=None, compute_loss=True):
        ...
        if self.config.state_tokenizer == 'resnet18':
            ...
            state_embeddings = self.state_encoder(states.reshape(-1, 2 , 720).type(torch.float32).contiguous()) # (batch * block_size, n_embd)
            state_embeddings = state_embeddings.reshape(states.shape[0], states.shape[1], self.config.n_embd) # (batch, block_size, n_embd)
        else:
            print('Not supported!')
        ...
        if actions is not None and self.model_type == 'GPT':
            ...
            B, N, C = actions.shape
            action_embeddings = self.action_embeddings(actions.view(B*N, C)).view(B, N, -1) # (batch, block_size, n_embd) 
        ...
        # position_embeddings = torch.gather(all_global_pos_emb, 1, torch.repeat_interleave(timesteps.to(self.device), self.config.n_embd, dim=-1)) + self.pos_emb[:, :token_embeddings.shape[1], :]

        # lines used for debugging
        # t_local = 0,1,2,...,15
        # t_global = 0,1,2,...,31 -> up until 2*t_max
        # timesteps = torch.arange(0, N).view(1,-1)
        # self.embed_timestep_global = self.embed_timestep_global.cpu()
        # self.embed_timestep_local = self.embed_timestep_local.cpu()
        
        # calculate global positional embedding (unique for each token)
        timesteps_global = torch.arange(timesteps.min(), timesteps.min()+2*timesteps.shape[1]).view(1,-1).to(self.device)
        ...
        if self.config.train_mode == 'e2e':
            ...
            # from the tokens of s_t and a_t, predict embedding of next state s_t+1
            B, N, D = x.shape
            # -1 to ignore the last s_t, a_t pair because we will not predict s_t+1 at the very end
            state_feat = x.reshape(B, int(N/2), -1)[:,:-1,:]
            state_preds = self.predict_state_token(state_feat)
            ...
            # from the tokens of s_t, s_t-1 a_t-1, predict delta in pose from t-1 to t
            B, N, D = x.shape
            # will not use the last tokens,
            pose_feat = torch.zeros(B, 3*int(N/2-1), D, device=self.device)
            pose_feat[:,::3,:] = x[:, ::2, :][:,:-1,:]
            pose_feat[:,1::3,:] = x[:, 1::2, :][:,:-1,:]
            pose_feat[:,2::3,:] = x[:, 2::2, :]
            ...
            feat = percep_feat.reshape(B, -1)  # reshape to a vector
            map_recon = self.map_decoder(feat)
            pose_preds = self.predict_pose_deep(x[:, ::2, :])
            # pose_preds[:,:,2:] = torch.tanh(pose_preds[:,:,2:])
        else:
            print('Not supported!')
        ...
        loss_act = None
        loss_states = None
        loss_translation_x = None
        loss_translation_y = None
        loss_angle = None
        if targets is not None:
            if self.config.train_mode == 'map':
                ...
                if compute_loss:
                    loss_act = self.criterion(actions, action_preds)
                    # for states, try to predict the next state based on current state and action
                    loss_states = self.criterion(x[:, 2::2, :], state_preds)
                    if self.config.use_pred_state:
                ...
                # loss over N timesteps
                if compute_loss:
                    # the :-1 ignores the pose prediction loss computed at the last element
                    # because the last state after sequence was garbage (zero) 
                    loss_translation_x = self.criterion(poses[:,:,0],pose_preds[:,:,0])
                    loss_translation_y = self.criterion(poses[:,:,1],pose_preds[:,:,1])
                    ...
                    loss = self.config.loc_x_loss_weight*loss_translation_x + \
