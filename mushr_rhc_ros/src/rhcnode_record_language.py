...
from std_msgs.msg import ColorRGBA, Empty, String, Float32MultiArray
...
class RHCNode(rhcbase.RHCBase):
    ...
    def start(self):
        ...
        # creating a language command for the first time
        self.sample_language_command()
    ...
    def sample_language_command(self):
        possible_speeds = np.array([0.0,0.5,1.0,1.5,2.0,2.5,3.0])
        self.speed = float(np.random.choice(possible_speeds))
        directions = ['left', 'right', 'straight', 'none']
        ...
        if direction_bias=='left':
            w_right = 100
            w_left = 0
        elif direction_bias=='right':
            w_right = 0
            w_left = 100
        elif direction_bias=='straight':
            w_right = 100
            w_left = 100
        else:
            w_right = 0
            w_left = 0
        rospy.set_param('/car/rhcontroller/cost_fn/penalize_left_w', w_left)
        rospy.set_param('/car/rhcontroller/cost_fn/penalize_right_w', w_right)
        if self.speed==0.0:
            # just a hack so it doesn't break the MPC controller with x/0 div
            # the output angle and vel will effectively be zero
            rospy.set_param('/car/rhcontroller/trajgen/desired_speed', 0.5)
        else:
            rospy.set_param('/car/rhcontroller/trajgen/desired_speed', self.speed)
        print('New language instruction: speed {} m/s | bias= {}'.format(self.speed, direction_bias))
        self.srv_reset_soft(msg=None)
        # self.srv_reset_hard(msg=None)


    def check_new_goal(self, next_traj, rate_hz):
        # condition if there is no goal currently set
        if self.goal_event.is_set() is False:
            # publish language cmd from this episode
            msg = Float32MultiArray()
            msg.data.append(self.speed)
            msg.data.append(self.direction_bias)
            self.language_bias.publish(msg)
            # publish episode status
            msg = String()
            msg.data = "first time"
        ...
        if self.inferred_pose_prev() is not None and next_traj is not None and self.time_started_goal is not None:
            ...
            if v < 0.05 and rospy.Time.now().to_sec() - self.time_started_goal.to_sec() > 1.0 and self.speed > 0.1:
        ...
        if self.time_started_goal is not None:
            if rospy.Time.now().to_sec() - self.time_started_goal.to_sec() > 90.0:
                ...
                self.send_initial_pose()
                self.sample_language_command()
                rospy.loginfo("Resetting pose of the car to default value anyways with timeout to increase diversity")
                return True
        return False
    ...
    def setup_pub_sub(self):
        ...
        # Publish the language command for that episode
        self.language_bias = rospy.Publisher("experiments/language_bias", Float32MultiArray, queue_size=1)
