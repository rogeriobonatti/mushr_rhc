...
def extract_bag_variables(bag_name):
    ...
    language = np.zeros((bag.get_message_count('experiments/language_bias'),3), dtype=np.float)
    ...
    for topic, msg, t in bag.read_messages(topics=['experiments/finished', 
        ...
        elif topic == 'experiments/language_bias':
            language[idx, 0] = t.to_sec()
            language[idx, 1] = msg.data[0] # speed
            language[idx, 2] = msg.data[1] # direction bias
        ...
        idx_dict[topic] += 1
    bag.close()
    return finished, language, angles, active, scans, poses, goals


def process_folder(folder, processed_dataset_path):
    bag_file_name = glob.glob(os.path.join(folder, '*.bag'))[0]
    finished, language, angles, active, scans, poses, goals = extract_bag_variables(bag_file_name)
    t_start = None
    t_end = None
    ep_num = 0
    for count, (t, event) in enumerate(finished):
        ...
        if t_start is None:
            ...
            if (int(event)==0 or int(event)==3) and (t_end - t_start)>5.0:
                ...
                # only get language instructions that are right before the end of the episode
                language_bias = language[(language[:,0]>=t_end-0.1) & (language[:,0]<t_end)]
                if goal.shape[0] == 0 or language_bias.shape[0] == 0:
                    # there's some issue with the goal or the language bias of this episode, so discard
                    pass
                else:
                    print("language bias:")
                    print(language_bias)
                    ...
                    if e_angles.shape[0]>100:
                        ...
                        np.savez(filename, ts=episode_ts, language_bias=language_bias, angles=episode_angles, lidars=episode_lidars, poses=episode_poses, goal=goal[0,1:3])
...
output_folder_name = 'processed_withpose3'
